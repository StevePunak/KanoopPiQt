...
{
    // Create I2C bus
    if((_fd = open(I2C::I2C_DEVICE, O_RDWR)) < 0)
    {
        KLog::sysLogText(KLOG_ERROR, QString("Failed to open the bus at %1.").arg(I2C::I2C_DEVICE));
        return false;
    }

    // Get I2C device, BMP280 I2C address is 0x76(108)
    ioctl(_fd, I2C_SLAVE, _address);

    connect(&_timer, &QTimer::timeout, this, &BMP280::handleTimer);
    _timer.start(1000);
...
void BMP280::handleTimer()
{
    // Read 24 bytes of data from address(0x88)
    char reg[1] = {0x88};
    write(_fd, reg, 1);
    char data[24] = {0};
    if(read(_fd, data, 24) != 24)
    {
        KLog::sysLogText(KLOG_ERROR, QString("Error : Input/output Error"));
        return;
    }
    ...
    double prsr = (p + (var1 + var2 + ((double)dig_P7)) / 16.0) / 100;

    // Output data to screen
    emit pressure(prsr);
    emit temperature(cTemp);
